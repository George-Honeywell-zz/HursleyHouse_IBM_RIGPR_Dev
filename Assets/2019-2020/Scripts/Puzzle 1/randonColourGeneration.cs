...
public class randonColourGeneration : MonoBehaviour
{
    ...
    IEnumerator ColourSequence()
    {
        ...
        for (int i = 0; i < currentDifficulty; i++)
        {
            ...
            if (sequence[i] == 0)
            {
                sequenceBlockRenderer.material.SetColor("_Color", Color.red);
                Debug.Log("Number " + (i + 1) + " in sequence is red");
                yield return new WaitForSeconds(1);
                sequenceBlockRenderer.material.SetColor("_Color", Color.white);
                yield return new WaitForSeconds(1);
            }
            if (sequence[i] == 1)
            {
                sequenceBlockRenderer.material.SetColor("_Color", Color.yellow);
            ...
            if (sequence[i] == 2)
            {
                sequenceBlockRenderer.material.SetColor("_Color", Color.blue);
            ...
            if (sequence[i] == 3)
            {
                sequenceBlockRenderer.material.SetColor("_Color", Color.green);
